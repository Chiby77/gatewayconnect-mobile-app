...
async function pullResource(resource: string, table: string): Promise<void> {
  ...
  for (const row of result.data) {
    if (resource === 'notifications') {
      database.runSync(`INSERT OR REPLACE INTO notifications (id, title, body, read_at, created_at) VALUES (?, ?, ?, ?, ?)`, row.id, row.title || '', row.body || '', row.read_at || null, row.created_at || new Date().toISOString());
    } else if (resource === 'groups') {
      database.runSync(`INSERT OR REPLACE INTO groups (id, name, description, location, updated_at) VALUES (?, ?, ?, ?, ?)`, row.id, row.name || '', row.description || '', row.location || null, row.updated_at || new Date().toISOString());
    } else if (resource === 'events') {
      database.runSync(`INSERT OR REPLACE INTO events (id, title, event_date, event_time, location, description, banner_url, category, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, row.id, row.title || '', row.event_date || '', row.event_time || '', row.location || '', row.description || null, row.banner_url || null, row.category || null, row.created_at || new Date().toISOString());
    } else if (resource === 'content_items') {
      const meta = typeof row.metadata === 'string' ? row.metadata : JSON.stringify(row.metadata || {});
      database.runSync(`INSERT OR REPLACE INTO content_items (id, type, title, body, metadata, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`, row.id, row.type || '', row.title || '', row.body || '', meta, row.created_at || new Date().toISOString(), row.updated_at || new Date().toISOString());
